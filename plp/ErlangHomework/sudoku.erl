...
%	false -> solve(Puzzle, attempts(Puzzle));
%	true -> {ok, Puzzle}
%end.

solve_valid(Puzzle) ->
    case lists:member(u, lists:flatten(Puzzle)) of
        false -> {ok, Puzzle};
        true -> case attempts(Puzzle) of
            [] -> {impossible, Puzzle};
            SolveMe -> solve(Puzzle, SolveMe)
        end
    end.
...
solve(Puzzle, PossibleNumbers) when length(PossibleNumbers) =:= 0 -> {notDone, Puzzle};
solve(Puzzle, PossibleNumbers) ->
    [H | T] = PossibleNumbers,
    NewPuzzle = subst_first(Puzzle, H),
    case solve(NewPuzzle) of
        {impossible, _} -> solve(Puzzle, T);
        {notDone, _} -> solve(Puzzle, T);
        {ok, BestPuzzle} -> {ok, BestPuzzle}
    end.
...
attempts_help(_Puzzle, {-1, -1, -1}) -> [];
attempts_help(Puzzle, {Row, Col, Box}) ->
	attempts_help2(remove(lists:nth(Row + 1, Puzzle), u), 
				   remove(lists:nth(Col + 1, transpose(Puzzle)), u), 
				   remove(lists:nth(Box + 1, get_submatrices(Puzzle)), u),
				   length(Puzzle)).

attempts_help2(Row, Col, Box, Length) ->
	lists:subtract(lists:seq(1, Length), remove_duplicates(lists:merge(lists:merge(Row, Col), Box))).

remove(L, Y) -> [X || X <- L, Y /= X].
...
find_first_blank([H|T], Row, Col) -> L2 = lists:flatten([H|T]),
	find_first_blank_help(L2, Row, Col, 0, length([H|T])).

find_first_blank_help(_L, _Row, _Col, Count, Length) when Count == Length * Length -> {-1, -1};
find_first_blank_help([H|T], Row, Col, Count, Length) -> Row2 = Count div Length, Col2 = Count rem Length,
	case H of
		u -> if
				(Row2 < Row) or (Col2 < Col) -> find_first_blank_help(T, Row, Col, Count + 1, Length);
				true -> Sqrt = round(math:sqrt(Length)),
						{Row2, Col2, (((Col2 div Sqrt) * Sqrt) + (Row2 div Sqrt))}
			end;
		_ -> find_first_blank_help(T, Row, Col, Count + 1, Length)
	end.
