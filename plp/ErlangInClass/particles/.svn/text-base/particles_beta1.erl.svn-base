...
particle_loop(Particle, FixedMass) ->
	receive
		{whereAreYou} ->
			...
			% FIXME: better to send tuple back to a listening process
			io:format("At (~.2f,~.2f)~n", [P2#particle.x, P2#particle.y]),
...
launch_particle(M) ->
	P = random_particle(),
	FM = fixed_mass(M),
	spawn(particles_beta1, particle_loop, [P,FM]).
