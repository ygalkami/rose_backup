...
force_server_loop(PartPids) ->
	receive
		...
		{ParticlePid, {updateNeeded}} ->
			...
			spawn(particles_beta5, force_reporter, [self(), ParticlePid, Others]),
			force_server_loop(PartPids);
...
launch_particles(N) ->
	FSPid = spawn(particles_beta5, force_server_loop, []),
	Pids = [spawn(particles_beta5, particle_loop, [random_particle(), FSPid]) || 
				_ <- lists:seq(1,N)],
...
periodic_print(Pid, Tick) ->
	spawn(particles_beta5, periodic_print_helper, [Pid, Tick]).
...
spawn_gui(ForceServer, PartPids) ->
	spawn(particles_beta5, gui_helper, [ForceServer, PartPids]).
