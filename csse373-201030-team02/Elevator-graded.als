-- CONSIDER: Include author names and a short description here.
...
-- Facts
--------------------------------------------------
-- CONSIDER: See notes in report about facts.
fact doorAlwaysOpen {
...
-- CONSIDER: What about constraint that arrows are only on when the doors are
--           open?
...
pred serviceFloor [t, t': Time, l: Lift] {
	...
	//remove it from the set
	-- CONSIDER: This implication is wrong.  It allows the requests set
	--           to change arbitrarily if the current floor is not requested.
	--           Recall that A implies B is true whenever A and B are both true
	--           _or_ when A is false regardless of B.
	l.currentFloor[t'] in l.requests[t] implies 
		l.requests[t'] = l.requests[t] - l.currentFloor[t']
}

-- CONSIDER: Once you've removed the extra facts, you'll need to strengthen
--           your predicates to ensure that only the relations that you want
--           to change are allowed to change.
