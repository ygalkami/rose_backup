...
main(int argc, char *argv[])
{
    ...
    while(1) {
      ...
      printf("What would you like to do (Echo, get, put, exit)?: ");
	...
	if (strcasecmp(input_string, "echo") == 0) {
		...
		printf("\n");
	} else if (strcasecmp(input_string, "get") == 0) {
		...
		fclose(inFile);
	} else  if (strcasecmp(input_string, "put") == 0) {
	  printf("Enter the name of the file you want to send: ");
	  scanf("%s", input_string);
	  inFile = fopen(input_string, "r");
	  
	  if (inFile == NULL) {
	    die_with_error("Enter a valid file");
	  }
	  send_file(inFile, input_string, sock, &serv_addr);
	}
   }	
}   
